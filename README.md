...
`trust-policy.json` 은 `nalbam/lambda-gurumi-bot` repo 의 OIDC 토큰을, `role-policy.json` 은 CloudFormation / Lambda / IAM / S3 / DynamoDB / API Gateway / CloudWatch Logs 권한을 (`lambda-gurumi-bot-*` 스코프) 포함합니다.
