...
| 클래스 | 역할 | 주요 메서드 |
|--------|------|------------|
| `Config` | 환경 변수 기반 설정 (17개 항목) | `validate()`, `get_reaction_emojis()` |
| `DynamoDBManager` | 컨텍스트 저장/조회, 중복 감지, 쓰로틀링 | `save_context()`, `get_context()`, `get_throttle_count()` |
...
| `kakao_handler` | (비활성) | Kakao 봇 진입점 (serverless.yml에서 주석 처리) |
| `handle_mention` | `app_mention` 이벤트 | 앱 멘션 처리 |
| `handle_message` | `message.im` 이벤트 | 다이렉트 메시지 처리 |
...
4. `serverless deploy`
5. Knowledge Base → Agent 연결 (최초 배포 시)
...
| `AGENT_ID` | (필수) | Bedrock Agent ID |
| `AGENT_ALIAS_ID` | (필수) | Bedrock Agent Alias ID |
| `DYNAMODB_TABLE_NAME` | `gurumi-ai-bot-dev` | DynamoDB 테이블명 |
| `ALLOWED_CHANNEL_IDS` | `None` | 허용 채널 (쉼표 구분, None=전체 허용) |
| `PERSONAL_MESSAGE` | AI 어시스턴트 | 페르소나 프롬프트 |
...
- Bedrock Agent는 AWS 콘솔에서 별도 관리 (CloudFormation 외부). `AGENT_ID`/`AGENT_ALIAS_ID` 환경 변수로 참조
